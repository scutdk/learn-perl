...
my $MAX_THREADS = 15;
my $work_q = Thread::Queue->new(1..28000);
my @work_threads = ();
...
#my $url = "http://www.capitalairlines.com.cn/frontend/groupbuying/order/grouporder!doQueryDetail.action?condition.id=$task";
        $url = "http://www.capitalairlines.com.cn/frontend/groupbuying/order/grouporder!passengerInfo.action?routesId=$task&cabinNum=6";
        my $res = $ua->get($url);
        unless($res->is_success()) {
