...
my $MAX_THREADS = 15;
my $work_q = Thread::Queue->new(3000..9000);
my @work_threads = ();
...
foreach (1..$MAX_THREADS) {
    push @work_threads, threads->create('worker');
}

#do some enqueue work
foreach (1..$MAX_THREADS) {
    $work_q->enqueue(undef);
}

while (threads->list() > 0) {
    foreach (@work_threads) {
        if ($_->is_running()) {
            sleep 3;
            next;
        }  
...
#my $url = "http://www.capitalairlines.com.cn/frontend/groupbuying/order/grouporder!doQueryDetail.action?condition.id=$task";
        my $url = "http://www.capitalairlines.com.cn/frontend/groupbuying/order/grouporder!passengerInfo.action?routesId=$task&cabinNum=6";
#print "http://www.capitalairlines.com.cn/frontend/groupbuying/order/grouporder!passengerInfo.action?routesId=$task&cabinNum=6\n";
        my $res = $ua->get($url);
        unless($res->is_success()) {
            print STDERR $res->code(), $res->as_string(),"\n";
            next;
        }
        my $page = $res->content();
        next unless $page =~ /机场/;
        my ($date) = $page =~ /(\d{4}-\d{2}-\d{2})/;
        my ($src, $dst) = $page =~ /(\S+机场)/gsm;
#my ($src, $dst) = (1,2);
        my ($price) = $page =~ /var\s*price\s*=\s*'([\d\.]+)'/;
        my ($airport) = $page =~ /var airport = '([\d\.]+)'/;
        my ($fuel) = $page =~ /var fuel = '([\d\.]+)'/;
        print "$task: $date, $src - $dst, $price, $airport, $fuel\n";
    }
}
...
#print $fh $jpgres->content();
            my %form = ("img" => encode_base64($jpgres->content()));
